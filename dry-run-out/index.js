...
// src/error_handle.ts
var generateErrorResponse = /* @__PURE__ */ __name((reason) => {
  switch (reason) {
    case 0 /* InvalidParm */:
      return {
...
// src/util.ts
var generateQuery = /* @__PURE__ */ __name((Method, hasCategory = false) => {
  switch (Method) {
    case 0 /* Get */:
      if (hasCategory) {
...
var db_fetch = /* @__PURE__ */ __name(async (c) => {
  const limit = c.req.query("limit");
  const type = c.req.query("type");
  const category = c.req.query("category");
  if (!type || !["onesan", "mesugaki", "osugaki"].includes(type)) {
    return c.json(
      generateErrorResponse(0 /* InvalidParm */),
      400
    );
  }
  if ((type === "mesugaki" || type === "osugaki") && !category) {
    return c.json(
      generateErrorResponse(0 /* InvalidParm */),
      400
    );
  }
  const count_num = limit !== void 0 ? Number(limit) : 1;
  ...
  ;
  try {
    const query = generateQuery(0 /* Get */, !!category);
    const prep = category ? c.env.DB.prepare(query).bind(type, category, count_num) : c.env.DB.prepare(query).bind(type, count_num);
    const word = await prep.all();
    if (!word.success) {
      return c.json(
        generateErrorResponse(1 /* QueryFailed */),
        500
      );
    }
    ...
    if (user_id && word.results && word.results.length > 0) {
      ...
      c.executionCtx.waitUntil(
        c.env.DB.prepare(usageQuery).bind(user_id, type).run()
      );
    }
    return c.json(word.results || []);
  } catch (e) {
    console.error(e);
    return c.json(
      generateErrorResponse(2 /* InternalError */),
      500
    );
  }
}, "db_fetch");
var db_ranking = /* @__PURE__ */ __name(async (c) => {
  const type = c.req.query("type");
  try {
    let prep;
    if (type) {
      if (!["onesan", "mesugaki", "osugaki"].includes(type)) {
        return c.json(generateErrorResponse(0 /* InvalidParm */), 400);
      }
      prep = c.env.DB.prepare("SELECT user_id, count FROM usages WHERE type = ? ORDER BY count DESC LIMIT 10;").bind(type);
    } else {
      prep = c.env.DB.prepare("SELECT user_id, SUM(count) as count FROM usages GROUP BY user_id ORDER BY count DESC LIMIT 10;");
    }
    const result = await prep.all();
    if (!result.success) {
      return c.json(generateErrorResponse(1 /* QueryFailed */), 500);
    }
    return c.json(result.results || []);
  } catch (e) {
    console.error(e);
    return c.json(generateErrorResponse(2 /* InternalError */), 500);
  }
}, "db_ranking");
var db_post = /* @__PURE__ */ __name(async (c) => {
  let body;
  try {
  ...
  if (typeof body.word !== "string" || body.word.trim() === "" || !["onesan", "mesugaki", "osugaki"].includes(body.type)) {
    return c.json(
      generateErrorResponse(0 /* InvalidParm */),
      400
    );
  }
  if ((body.type === "mesugaki" || body.type === "osugaki") && !body.category) {
    return c.json(
      generateErrorResponse(0 /* InvalidParm */),
      400
    );
  ...
  try {
    const query = generateQuery(1 /* Post */);
    const prep = c.env.DB.prepare(query).bind(body.word.trim(), body.type, body.category || null);
...
var db_delete = /* @__PURE__ */ __name(async (c) => {
  ...
  try {
    const query = generateQuery(2 /* Delete */);
    const prep = c.env.DB.prepare(query).bind(id_num);
    const result = await prep.run();
    if (!result.success) {
      return c.json(
        generateErrorResponse(1 /* QueryFailed */),
...
// src/mod/api.ts
async function fetchVocabulary(type, category, user_id, baseUrl) {
  let url = `${baseUrl}/vocabulary?type=${type}`;
  if (category) {
...
__name(fetchVocabulary, "fetchVocabulary");
async function fetchRanking(type, baseUrl) {
...
__name(fetchRanking, "fetchRanking");
async function addVocabulary(type, word, category, baseUrl) {
  const url = `${baseUrl}/vocabulary`;
...
__name(addVocabulary, "addVocabulary");

// src/mod/flavor.ts
var generateRandNum = /* @__PURE__ */ __name((min, max) => {
  if (max < min) throw Error("max must be greater than or equal to min");
  return Math.floor(Math.random() * (max - min + 1) + min);
}, "generateRandNum");
var generateTilde = /* @__PURE__ */ __name(() => "\uFF5E".repeat(generateRandNum(3, 7)), "generateTilde");
var generateHeart = /* @__PURE__ */ __name(() => "\u2661".repeat(generateRandNum(2, 5)), "generateHeart");
var generateSmallO = /* @__PURE__ */ __name(() => "\u3049".repeat(generateRandNum(2, 6)), "generateSmallO");
var getOnesanPrefix = /* @__PURE__ */ __name(() => {
...
var getOnesanSuffix = /* @__PURE__ */ __name((choice) => {
  if (!choice || !Object.keys(EndChoice).includes(choice)) {
    const keys = Object.keys(EndChoice);
    choice = keys[generateRandNum(0, keys.length - 1)];
  }
  const base = EndChoice[choice];
  return `${base}${generateTilde()}\u3093${generateHeart()}`;
}, "getOnesanSuffix");
var getMesugakiBatouSuffix = /* @__PURE__ */ __name(() => {
  return generateHeart();
}, "getMesugakiBatouSuffix");
var getMesugakiWakaraseSuffix = /* @__PURE__ */ __name(() => {
  return `${generateSmallO()}\u304A${generateHeart()}`;
}, "getMesugakiWakaraseSuffix");

// src/mod/command/onesan.ts
var onesan = {
  data: {
    name: "onesan",
    description: "\u304A\u59C9\u3055\u3093\u304C\u558B\u308A\u307E\u3059",
    options: [
      {
        name: "suffix",
        description: "\u6587\u672B\u306E\u30D1\u30BF\u30FC\u30F3\u3092\u9078\u629E\uFF08\u672A\u6307\u5B9A\u306A\u3089\u30E9\u30F3\u30C0\u30E0\uFF09",
        type: 3,
        // STRING type in Discord API
        required: false,
        choices: [
  ...
  execute: /* @__PURE__ */ __name(async (interaction, env) => {
    const options = interaction.data?.options || [];
    const suffixOption = options.find((opt) => opt.name === "suffix");
    const suffixChoice = suffixOption ? suffixOption.value : null;
    const user_id = interaction.member?.user?.id || interaction.user?.id;
    const word = await fetchVocabulary("onesan", null, user_id, env.API_BASE_URL);
    if (!word) {
      return { type: 4, data: { content: "\u8A9E\u5F59\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3067\u3057\u305F\u3002" } };
    }
...
var onesan_default = onesan;

// src/mod/command/mesugaki.ts
var mesugaki = {
  data: {
    name: "mesugaki",
    description: "\u30E1\u30B9\u30AC\u30AD\u304C\u558B\u308A\u307E\u3059",
    options: [
      {
        name: "category",
        description: "\u30AB\u30C6\u30B4\u30EA\u3092\u9078\u629E",
  ...
  execute: /* @__PURE__ */ __name(async (interaction, env) => {
    const options = interaction.data?.options || [];
    const catOption = options.find((opt) => opt.name === "category");
    const category = catOption ? catOption.value : null;
    const user_id = interaction.member?.user?.id || interaction.user?.id;
    const word = await fetchVocabulary("mesugaki", category, user_id, env.API_BASE_URL);
    if (!word) {
      return { type: 4, data: { content: "\u8A72\u5F53\u30AB\u30C6\u30B4\u30EA\u306E\u8A9E\u5F59\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3067\u3057\u305F\u3002" } };
    }
...
var mesugaki_default = mesugaki;

// src/mod/command/osugaki.ts
var osugaki = {
  data: {
    name: "osugaki",
    description: "\u30AA\u30B9\u30AC\u30AD\u304C\u558B\u308A\u307E\u3059",
    options: [
      {
        name: "category",
        description: "\u30AB\u30C6\u30B4\u30EA\u3092\u9078\u629E",
  ...
  execute: /* @__PURE__ */ __name(async (interaction, env) => {
    ...
    const category = catOption ? catOption.value : null;
    const user_id = interaction.member?.user?.id || interaction.user?.id;
    const word = await fetchVocabulary("osugaki", category, user_id, env.API_BASE_URL);
    if (!word) {
      return { type: 4, data: { content: "\u8A72\u5F53\u30AB\u30C6\u30B4\u30EA\u306E\u8A9E\u5F59\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3067\u3057\u305F\u3002" } };
    }
...
var osugaki_default = osugaki;

// src/mod/command/onesan_add.ts
var onesanAdd = {
  data: {
    name: "onesan_add",
  ...
  execute: /* @__PURE__ */ __name(async (interaction, env) => {
    const options = interaction.data?.options || [];
    const wordOption = options.find((opt) => opt.name === "word");
    if (!wordOption) return { type: 4, data: { content: "Error: word is required" } };
    const success = await addVocabulary("onesan", wordOption.value, null, env.API_BASE_URL);
    if (!success) {
...
var onesan_add_default = onesanAdd;

// src/mod/command/mesugaki_add_batou.ts
var mesugakiAddBatou = {
  data: {
    name: "mesugaki_add_batou",
    description: "\u30E1\u30B9\u30AC\u30AD\uFF08\u7F75\u5012\uFF09\u306E\u8A9E\u5F59\u3092\u8FFD\u52A0\u3057\u307E\u3059",
    options: [
      {
        name: "word",
        description: "\u8FFD\u52A0\u3059\u308B\u8A00\u8449",
  ...
  execute: /* @__PURE__ */ __name(async (interaction, env) => {
    ...
    const wordOption = options.find((opt) => opt.name === "word");
    if (!wordOption) return { type: 4, data: { content: "Error: word is required" } };
    const success = await addVocabulary("mesugaki", wordOption.value, "batou", env.API_BASE_URL);
    if (!success) {
      return { type: 4, data: { content: "\u8A9E\u5F59\u306E\u8FFD\u52A0\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002API\u30A8\u30E9\u30FC\u306A\u3069\u3092\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002", flags: 64 } };
    }
    return { type: 4, data: { content: `\u30E1\u30B9\u30AC\u30AD\uFF08\u7F75\u5012\uFF09\u306E\u8A9E\u5F59\u3092\u8FFD\u52A0\u3057\u307E\u3057\u305F\uFF01
Word: \`${wordOption.value}\``, flags: 64 } };
  }, "execute")
};
var mesugaki_add_batou_default = mesugakiAddBatou;

// src/mod/command/mesugaki_add_wakarase.ts
var mesugakiAddWakarase = {
  data: {
    name: "mesugaki_add_wakarase",
    description: "\u30E1\u30B9\u30AC\u30AD\uFF08\u308F\u304B\u3089\u305B\uFF09\u306E\u8A9E\u5F59\u3092\u8FFD\u52A0\u3057\u307E\u3059",
    options: [
      {
        name: "word",
        description: "\u8FFD\u52A0\u3059\u308B\u8A00\u8449",
  ...
  execute: /* @__PURE__ */ __name(async (interaction, env) => {
    ...
    const wordOption = options.find((opt) => opt.name === "word");
    if (!wordOption) return { type: 4, data: { content: "Error: word is required" } };
    const success = await addVocabulary("mesugaki", wordOption.value, "wakarase", env.API_BASE_URL);
    if (!success) {
      return { type: 4, data: { content: "\u8A9E\u5F59\u306E\u8FFD\u52A0\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002API\u30A8\u30E9\u30FC\u306A\u3069\u3092\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002", flags: 64 } };
    }
    return { type: 4, data: { content: `\u30E1\u30B9\u30AC\u30AD\uFF08\u308F\u304B\u3089\u305B\uFF09\u306E\u8A9E\u5F59\u3092\u8FFD\u52A0\u3057\u307E\u3057\u305F\uFF01
Word: \`${wordOption.value}\``, flags: 64 } };
  }, "execute")
};
var mesugaki_add_wakarase_default = mesugakiAddWakarase;

// src/mod/command/osugaki_add_batou.ts
var osugakiAddBatou = {
  data: {
    name: "osugaki_add_batou",
    description: "\u30AA\u30B9\u30AC\u30AD\uFF08\u7F75\u5012\uFF09\u306E\u8A9E\u5F59\u3092\u8FFD\u52A0\u3057\u307E\u3059",
    options: [
      {
        name: "word",
        description: "\u8FFD\u52A0\u3059\u308B\u8A00\u8449",
  ...
  execute: /* @__PURE__ */ __name(async (interaction, env) => {
    ...
    const wordOption = options.find((opt) => opt.name === "word");
    if (!wordOption) return { type: 4, data: { content: "Error: word is required" } };
    const success = await addVocabulary("osugaki", wordOption.value, "batou", env.API_BASE_URL);
    if (!success) {
      return { type: 4, data: { content: "\u8A9E\u5F59\u306E\u8FFD\u52A0\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002API\u30A8\u30E9\u30FC\u306A\u3069\u3092\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002", flags: 64 } };
    }
...
var osugaki_add_batou_default = osugakiAddBatou;

// src/mod/command/osugaki_add_wakarase.ts
var osugakiAddWakarase = {
  data: {
  ...
  execute: /* @__PURE__ */ __name(async (interaction, env) => {
    ...
    const wordOption = options.find((opt) => opt.name === "word");
    if (!wordOption) return { type: 4, data: { content: "Error: word is required" } };
    const success = await addVocabulary("osugaki", wordOption.value, "wakarase", env.API_BASE_URL);
    if (!success) {
      return { type: 4, data: { content: "\u8A9E\u5F59\u306E\u8FFD\u52A0\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002API\u30A8\u30E9\u30FC\u306A\u3069\u3092\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002", flags: 64 } };
    }
...
var osugaki_add_wakarase_default = osugakiAddWakarase;

// src/mod/command/ranking.ts
var ranking = {
  data: {
    name: "ranking",
  ...
  execute: /* @__PURE__ */ __name(async (interaction, env) => {
    ...
    const rankingData = await fetchRanking(type, env.API_BASE_URL);
...
var commands = /* @__PURE__ */ new Map([
  [onesan_default.data.name, onesan_default],
  [mesugaki_default.data.name, mesugaki_default],
  [osugaki_default.data.name, osugaki_default],
  [onesan_add_default.data.name, onesan_add_default],
  [mesugaki_add_batou_default.data.name, mesugaki_add_batou_default],
  [mesugaki_add_wakarase_default.data.name, mesugaki_add_wakarase_default],
  [osugaki_add_batou_default.data.name, osugaki_add_batou_default],
  [osugaki_add_wakarase_default.data.name, osugaki_add_wakarase_default],
  [ranking_default.data.name, ranking_default]
]);
var app = new Hono2();
app.onError((err, c) => {
  console.error(`${err}`);
...
app.get("/", (c) => {
  return c.text("dac-bot-integrated API");
});
...
app.post("/discord", async (c) => {
  ...
  const isValidRequest = await (0, import_discord_interactions.verifyKey)(body, signature, timestamp, c.env.DISCORD_PUBLIC_KEY);
  if (!isValidRequest) {
  ...
  if (interaction.type === import_discord_interactions.InteractionType.PING) {
    return new Response(JSON.stringify({ type: import_discord_interactions.InteractionResponseType.PONG }), {
  ...
  if (command) {
    try {
      ...
      return c.json({ type: 4, data: { content: "\u30A8\u30E9\u30FC\u304C\u767A\u751F\u3057\u307E\u3057\u305F\u3002" } });
    }
  }
