...
import { music_resolve } from "./handler/music";
...
app.get("/", (c) => c.text("dac-bot-mod-integrated with discord-interaction api"));
app.get("/vocabulary", async (c) => await db_fetch(c));
app.get("/ranking", async (c) => await db_ranking(c));
app.get("/music", async (c) => await music_resolve(c));
app.post("/vocabulary", async (c) => await db_post(c));
app.delete("/vocabulary/:id", async (c) => await db_delete(c));
...
app.post("/", async (c) => {
  ...
  if (!signature || !timestamp) return c.text("Missing signature in header", 401);

  const isValidRequest = await verifyKey(body, signature, timestamp, c.env.DISCORD_PUBLIC_KEY);
  if (!isValidRequest) return c.text("Verifing key was faild", 401);
  ...
  if (command) {
    try {
      return c.json(await command.execute(interaction, c.env, c.executionCtx));
    } catch (e) {
